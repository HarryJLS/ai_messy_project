...
| 通用 | 抽取公共逻辑、架构调整 |
| 控制流 | if嵌套>3层优化、for嵌套>2层优化、条件表达式>3个拆分、循环内查询优化 |
...
| 抽取公共逻辑 | 重复代码块 | 提供函数签名 |
| if嵌套优化 | 嵌套深度 > 3 层 | 使用提前 return 重构 |
| for嵌套优化 | 嵌套深度 > 2 层 | 抽取方法或使用 Map |
| 条件表达式拆分 | 条件数 > 3 个 | 提取局部布尔变量 |
| 循环内查询优化 | 循环内 SQL/API 调用 | 批量查询 + Map 转换 |
